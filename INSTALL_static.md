...
Before installing your cluster, make sure you set the `INVENTARIO_A_USAR` environment variable to `my_cluster_name`.
...
Make sure you set the `ENTORNO_A_USAR` environment variable to `static`.

```
export ENTORNO_A_USAR=static
cd ~/ansible-hortonworks*/ && bash instalar_cluster.sh
```
...
- Prepare the nodes: `preparar_nodos.sh`
- Install Ambari: `instalar_ambari.sh`
- Configure Ambari: `configurar_ambari.sh`
- Apply Blueprint: `aplicar_blueprint.sh`
- Post Install: `post_instalacion.sh`
